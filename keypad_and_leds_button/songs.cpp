...
void darthVader(int pin) {
  int tempo = 108;
  // sizeof gives the number of bytes, each int value is composed of two bytes (16 bits)
  // there are two values per note (pitch and duration), so for each note there are four bytes
  int notes = sizeof(darthVaderMelody) / sizeof(darthVaderMelody[0]) / 2;
  ...
  for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
    ...
    divider = darthVaderMelody[thisNote + 1];
    if (divider > 0) {
    ...
    tone(pin, darthVaderMelody[thisNote], noteDuration*0.9);
...
void cantina(int pin) {
  ...
  for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
    ...
    // we only play the note for 90% of the duration, leaving 10% as a pause
    tone(pin, cantinaMelody[thisNote], noteDuration*0.9);
...
void nggyu(int pin)
{
  int size = sizeof(nggyu_durations) / sizeof(int);

  for (int note = 0; note < size; note++) {
    //to calculate the note duration, take one second divided by the note type.
    //e.g. quarter note = 1000 / 4, eighth note = 1000/8, etc.
    int duration = 1000 / nggyu_durations[note];
    tone(pin, nggyu_melody[note], duration);
...
void thelionsleeptonight(int pin) {
  // sizeof gives the number of bytes, each int value is composed of two bytes (16 bits)
  // there are two values per note (pitch and duration), so for each note there are four bytes
  int notes = sizeof(tlst_melody) / sizeof(tlst_melody[0]) / 2;
  int tempo = 122;
  // this calculates the duration of a whole note in ms
  int wholenote = (60000 * 4) / tempo;
  ...
  for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
    ...
    // calculates the duration of each note
    divider = tlst_melody[thisNote + 1];
    if (divider > 0) {
      // regular note, just proceed
      noteDuration = (wholenote) / divider;
    } else if (divider < 0) {
    ...
    // we only play the note for 90% of the duration, leaving 10% as a pause
    tone(pin, tlst_melody[thisNote], noteDuration * 0.9);
